...
check_sudo_access() {
  ...
  elif timeout 120 sudo -v 2>/dev/null; then
...
Environment Variables:
  ...
  NVIDIA_API_KEY         NVIDIA Cloud Function API Key to access models at build.nvidia.com
                         Can be set in environment or will be prompted if not set
  HF_TOKEN               HuggingFace token to download the models for customization from HuggingFace 
                         Can be set in environment or will be prompted if not set
...
Examples:
  # Using latest available version from repo with chart defaults (simplest)
  $0
  
  # Using latest available version from repo with custom values
  $0 --values-file /path/to/values1.yaml
  
  # Using specific chart version from repo
  $0 --helm-chart-version 25.9.0
  
  # Using specific chart version with custom values
  $0 --helm-chart-version 25.9.0 --values-file /path/to/values1.yaml
...
validate_args() {
  ...
  if [[ ${#ADDITIONAL_VALUES_FILES[@]} -gt 0 ]]; then
    log "Using ${#ADDITIONAL_VALUES_FILES[@]} custom values file(s) for deployment configuration."
  else
...
check_prereqs() {
  ...
  while IFS= read -r gpu; do
    ...
    if [[ "$gpu" == *"A100"*"80GB"* ]] || [[ "$gpu" == *"H100"*"80GB"* ]] || [[ "$gpu" == *"6000"* ]] || [[ "$gpu" == *"5880"* ]] || [[ "$gpu" == *"H200"* ]]; then
...
setup_ngc_and_helm() {
  ...
  kubectl delete secret nvcrimagepullsecret -n "$NAMESPACE" --ignore-not-found=true || warn "Failed to delete nvcrimagepullsecret secret"

  # Delete secret if it exists
  kubectl delete secret ngc-api -n "$NAMESPACE" --ignore-not-found=true || warn "Failed to delete ngc-api secret"

  # Delete secret if it exists
  kubectl delete secret nvidia-api -n "$NAMESPACE" --ignore-not-found=true || warn "Failed to delete nvidia-api secret"
...
setup_nvcf_key() {
  ...
  if kubectl get secret nvidia-api -n "$NAMESPACE" &>/dev/null; then
    log "Deleting existing nvidia-api secret..."
    kubectl delete secret nvidia-api -n "$NAMESPACE"
  fi
...
# Get the HF_TOKEN and store it in Kubernetes secret for use by Customizer to download models from HF
setup_hf_secret() {
  if [[ -z "${HF_TOKEN-}" || -z "$HF_TOKEN" ]]; then
    read -rsp "Enter your HuggingFace token: " HF_TOKEN_INPUT
    echo
  ...
  if kubectl get secret hf-token -n "$NAMESPACE" &>/dev/null; then
    log "Deleting existing hf-token secret..."
...
download_helm_chart() {
  ...
  if [[ -n "$HELM_CHART_VERSION" ]]; then
    ...
    helm repo add nmp https://helm.ngc.nvidia.com/nvidia/nemo-microservices \
      --username='$oauthtoken' \
      --password="$NGC_API_KEY" || die "Failed to add helm repository"
